<template>
    <UIModal v-slot="{ setOpen }" modal-id="delete-form-modal">
        <div class="flex flex-col text-center gap-2">
            ...
            <button @click="doDeleteForm(setOpen)" class="btn btn-error mt-4">Yes, Delete</button>
        </div>
    </UIModal>
</template>
...
const { mutate, pending } = useMutate(deleteForm);

async function doDeleteForm(setOpen: (state: boolean) => void) {
