<template>
    <UIModal v-slot="{ setOpen }" modal-id="delete-comment-modal">
        <div class="flex flex-col text-center gap-2">
            ...
            <button @click="doDeleteComment(setOpen)" class="btn btn-error mt-4" :class="{ 'btn-loading': pending }">
                Yes, Delete
            </button>
        </div>
    </UIModal>
...
const { mutate, pending } = useMutate(deleteComment);

async function doDeleteComment(setOpen: (state: boolean) => void) {
